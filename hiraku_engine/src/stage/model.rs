...
impl StageMaterial {
    pub(super) fn validate(&self) -> bool {
        ...
        self.base_color.is_none_or(|v| v.into_iter().all(unit))
            && self.metallic.is_none_or(unit)
            && self.roughness.is_none_or(unit)
            && match self.alpha {
                Some(StageAlpha::Mask { cutoff }) => unit(cutoff),
                _ => true,
            }
    ...
    pub(super) fn apply(&self, material: &mut StandardMaterial) {
        ...
        if self.uv_scale.is_some() || self.uv_offset.is_some() {
            material.uv_transform = bevy::math::Affine2::from_scale_angle_translation(
                self.uv_scale.map(Vec2::from).unwrap_or(Vec2::ONE),
                0.0,
...
impl StageLight {
    ...
    pub(super) fn spawn(&self, commands: &mut Commands, parent: Entity) {
        ...
        commands.spawn((
            ...
            self.pose.transform(),
            ChildOf(parent),
            super::runtime::StageSurface,
            super::views::spatial_layer(),
        ));
    }
}
