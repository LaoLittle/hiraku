...
pub(crate) fn initialize_runtime_storage(config: Res<crate::RuntimeLaunchConfig>) {
    hiraku_storage::initialize_runtime(
...
pub(crate) fn poll_runtime_storage(
    ...
    if !matches!(
        status,
        RuntimeStorageStatus::Ready | RuntimeStorageStatus::Failed(_)
    ) {
        redraw.request();
    }
    ...
    if let RuntimeStorageStatus::Failed(error) = &status {
        ...
        if let Some(mut frontend) = frontend {
            frontend.notice = Some(format!("Storage failed: {error}"));
        }
    } else if status == RuntimeStorageStatus::Ready
        && previous.as_ref() == Some(&RuntimeStorageStatus::Writing)
    {
