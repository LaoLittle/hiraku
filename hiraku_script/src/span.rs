...
thread_local! {
    static GLOBAL_SESSION: GlobalSession = GlobalSession::new();
}

pub fn with_global_session_init<F: FnOnce() -> R, R>(f: F) -> R {
    GLOBAL_SESSION.with(|_| f())
}
...
macro_rules! pre_define_symbol {
    (Symbols { $($str:expr => $sym:ident: $num:expr);* $(;)? }) => {
        $(pub const $sym: $crate::span::symbol::Symbol = $crate::span::symbol::Symbol::new($num);)*
...
mod tests {
    ...
    use super::{BOOLEAN, INT8, INT16, INT32, INT64, Interner, with_global_session_init};
