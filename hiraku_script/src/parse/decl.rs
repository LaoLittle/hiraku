...
use crate::ast::decl::{FunArg, FunDecl, ImportDecl, PackageDecl};
use crate::span::Span;
...
impl<'a> Parser<'a> {
    ...
    fn parse_arg_list(&mut self) -> Vec<FunArg> {
        ...
        let mut push_arg = |parser: &mut Parser, vararg: bool| -> Result<(), ()> {
            ...
            let tk = parser.advance_token_skip_nl();
            if tk == Token::Ident {
                ...
                parser._expect_token(tk, Token::Ident);
                Err(())
            }
        };
        ...
        loop {
            ...
            if let Token::Comma = self.peek_token_skip_nl() {
                self.bump();
            } else {
                break;
            }
